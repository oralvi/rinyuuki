...
import rin
from rin import logger, util

class ResObj:
    def __init__(self, res_path):
        res_dir = os.path.expanduser(rin.config.RES_DIR)
    ...
    def url(self):
        ...
        return urljoin(rin.config.RES_URL, pathname2url(self.__path))

    @property
    def path(self):
        """资源文件的路径，供bot内部使用"""
        return os.path.join(rin.config.RES_DIR, self.__path)
