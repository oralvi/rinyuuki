...
import rin
from rin.typing import CQEvent, Message, Union
...
async def delete_msg(ev: CQEvent):
    try:
        await rin.get_bot().delete_msg(self_id=ev.self_id, message_id=ev.message_id)
    except ActionFailed as e:
        rin.logger.error(f'撤回失败 retcode={e.retcode}')
    except Exception as e:
        rin.logger.exception(e)
...
async def silence(ev: CQEvent, ban_time, skip_su=True):
    try:
        if skip_su and ev.user_id in rin.config.SUPERUSERS:
            return
        await rin.get_bot().set_group_ban(self_id=ev.self_id, group_id=ev.group_id, user_id=ev.user_id, duration=ban_time)
    except ActionFailed as e:
        rin.logger.error(f'禁言失败 retcode={e.retcode}')
    except Exception as e:
        rin.logger.exception(e)
