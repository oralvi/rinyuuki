...
import rin
from rin import log, priv, trigger
...
_service_config_dir = os.path.expanduser('~/.rin/service_config/')
...
class Service:
    ...
    储存位置：
    `~/.rin/service_config/{ServiceName}.json`
    """
    def __init__(self, name, use_priv=None, manage_priv=None, enable_on_default=None, visible=None,
        ...
        self.logger = log.new_logger(name, rin.config.DEBUG)
...
sulogger = log.new_logger('sucmd', rin.config.DEBUG)
...
def sucmd(name, force_private=True, **kwargs) -> Callable:
    ...
    def deco(func) -> Callable:
        ...
        async def wrapper(session: CommandSession):
            if session.event.user_id not in rin.config.SUPERUSERS:
                return
