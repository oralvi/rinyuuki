...
'''
sv = rin.Service('涩图', bundle='pcr娱乐', help_=HELP_MSG)
...
if not os.path.exists(pathcfg):
	try:
		with open(pathcfg, 'w') as cfgf:
			...
			rin.logger.error('[WARNING]未找到配置文件，已根据默认配置模板创建，请打开插件目录内config.json查看和修改。')
	except:
		rin.logger.error('[ERROR]创建配置文件失败，请检查插件目录的读写权限及是否存在config.json。')
		traceback.print_exc()
...
if not os.path.exists(glpath):
	try:
		with open(glpath, 'w') as cfggl:
			...
			rin.logger.error('[WARNING]未找到黑白名单文件，已根据默认黑白名单模板创建。')
	except:
		rin.logger.error('[ERROR]创建黑白名单文件失败，请检查插件目录的读写权限。')
		traceback.print_exc()
...
if not os.path.exists(gpcfgpath):
	try:
		with open(gpcfgpath, 'w') as gpcfg:
			...
			rin.logger.error('[WARNING]未找到群个体设置文件，已创建。')
	except:
		rin.logger.error('[ERROR]创建群个体设置文件失败，请检查插件目录的读写权限。')
		traceback.print_exc()
...
# 设置limiter
tlmt = rin.util.DailyNumberLimiter(get_config('base', 'daily_max'))
flmt = rin.util.FreqLimiter(get_config('base', 'freq_limit'))


def check_lmt(uid, num, gid):
	if uid in rin.config.SUPERUSERS:
		return 0, ''
...
async def send_msg(msg_list, ev):
	...
	if not get_config('base', 'enable_forward_msg'):
		for msg in msg_list:
			try:
				result_list.append(await rin.get_bot().send(ev, msg))
			except:
				rin.logger.error('[ERROR]图片发送失败')
				await rin.get_bot().send(ev, f'涩图太涩,发不出去力...')
			await asyncio.sleep(1)
		return result_list
	else:
		forward_msg = render_forward_msg(msg_list)
		try:
			await rin.get_bot().send_group_forward_msg(group_id=ev.group_id, messages=forward_msg)
		except:
			traceback.print_exc()
			rin.logger.error('[ERROR]图片发送失败')
			await rin.get_bot().send(ev, f'涩图太涩,发不出去力...')
...
async def send_setu(bot, ev):
	uid = ev['user_id']
	gid = ev['group_id']
	is_su = rin.priv.check_priv(ev, rin.priv.SUPERUSER)
