...
async def query_search(keyword):
    ...
    rin.logger.info(f"[INFO]搜索结果数量{len(data['illusts'])}")
    return image_list
...
async def download_acggov_image(url: str):
    rin.logger.info(f'[INFO]acggov downloading image{url}')
    try:
        async with aiohttp.ClientSession(headers=acggov_headers) as session:
...
async def download_pixiv_image(url: str, id):
    rin.logger.info(f'[INFO]acggov downloading pixiv image{url}')
    headers = {
...
async def acggov_fetch_process():
    global ranking_date
    if get_config('acggov', 'mode') == 2:
        rin.logger.info('[INFO]fetch acggov setu')
        for _ in range(10):
        ...
        if date != ranking_date:
            rin.logger.info('[INFO]fetch acggov ranking setu')
            for i in range(25):
