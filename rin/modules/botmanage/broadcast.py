...
import rin
from rin.service import sucmd
from rin.typing import CommandSession, CQHttpError
    ...
    for sid in rin.get_self_ids():
        ...
        for g in gl:
            ...
            try:
                ...
                rin.logger.info(f'群{g} 投递广播成功')
            except Exception as e:
                rin.logger.error(f'群{g} 投递广播失败：{type(e)}')
                try:
                    await session.send(f'群{g} 投递广播失败：{type(e)}')
                except Exception as e:
                    rin.logger.critical(f'向广播发起者进行错误回报时发生错误：{type(e)}')
